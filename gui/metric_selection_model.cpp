...
void MetricSelectionModel::parseData(QIODevice &io) {
    ...
    while (!stream.atEnd()) {
        ...
        while (match.hasMatch()) {
            ...
            QRegularExpression reMetric("Metric #(\\d+): ([^\(]*)");
...
inline QString stringFromHash(QHash<QString, QList<QPair<int,int>>> hash) {
    QString result;
    for (auto it = hash.cbegin(); it != hash.cend(); ++it) {
        result += it.key() + ": ";
        for (auto &p : it.value()) {
            result += QString("[%1,%2],").arg(p.first).arg(p.second);
        }
        result += "; ";
    }
    return result;
...
void MetricSelectionModel::generateMetricList() const {
    ...
    qDebug() << "--pframes=\"" + stringFromHash(metricsFrame) + "\" --pdrawcalls=\""
             + stringFromHash(metricsCall) + "\"";
}
